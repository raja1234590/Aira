...
export default function NewRulePage() {
  ...
  // Smart navigation: check if user came from workspace or hub
  const handleCancel = useCallback(() => {
    // Check if there's a referrer or if we should default to workspace
    const referrer = document.referrer;
    if (referrer && referrer.includes('/workspace')) {
      router.push(ROUTES.WORKSPACE);
    } else if (referrer && referrer.includes('/')) {
      router.push(ROUTES.HUB);
    } else {
      // Default to workspace as it's the most common entry point for rule creation
      router.push(ROUTES.WORKSPACE);
    }
  }, [router]);
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <button
            type="button"
            onClick={handleCancel}
            className="flex h-10 items-center justify-center rounded-lg px-4 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
            aria-label="Cancel"
          >
        ...
        <div className="fixed bottom-0 left-0 right-0 border-t border-border bg-background px-5 py-4">
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={!canSave}
              className={cn(
                'flex-1 rounded-2xl py-6 text-base font-semibold',
                !canSave && 'opacity-50',
              )}
            >
